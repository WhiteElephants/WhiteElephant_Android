...
/**
 * Created by WindPush on 15/6/23.
 */
public class ProgressLoadingView extends View {
    ...
    public ProgressLoadingView (Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取自定义属性和默认值
        circleColor = mTypedArray.getColor(R.styleable.CircleProgressBar_circleColor, Color.TRANSPARENT);
        circleProgressColor = mTypedArray.getColor(R.styleable.CircleProgressBar_circleProgressColor, Color.WHITE);
        textColor = mTypedArray.getColor(R.styleable.CircleProgressBar_percentTextColor, Color.WHITE);
        textSize = mTypedArray.getDimension(R.styleable.CircleProgressBar_percentTextSize, 15);
        circleWidth = mTypedArray.getDimension(R.styleable.CircleProgressBar_percentCircleWidth, 15);
        max = mTypedArray.getInteger(R.styleable.CircleProgressBar_percentMax, 100);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int radius = (int) (centre - circleWidth / 2-20); //圆环的半径
        ...
        int percent = (int) (progress * 100f / max);  //中间的进度百分比，先转换成float在进行除法运算，不然都为0
        /**
         * 画进度百分比
         */
        if (textIsDisplayable && percent != 0 && style == STROKE) {
            ...
            canvas.drawText(percentStr, centre - textWidth / 2, centre + textSize / 2, paint); //画出进度百分比
        }
    ...
    public String toString() {
        return "CircleProgressBar{" +
                ...
                ", max=" + max +
                ", progress=" + progress +
                ", textIsDisplayable=" + textIsDisplayable +
                ", style=" + style +
                '}';
    }
}
